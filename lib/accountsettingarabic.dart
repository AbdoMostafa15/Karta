...
import 'package:karta/changepasswordarabic.dart';
import 'package:karta/home.dart';
import 'package:karta/homearabic.dart';
import 'package:karta/mainarabic.dart';

class AccountSettingsScreenarabic extends StatelessWidget {
  const AccountSettingsScreenarabic({super.key});
    ...
    if (user != null) {
      try {
        ...
        Navigator.pushAndRemoveUntil(
          context,
          MaterialPageRoute(builder: (context) => const Homearabic()),
          (route) => false,
        );
      } on FirebaseAuthException catch (e) {
        if (e.code == 'الرجاء تسجيل الدخول مجددا') {
          ...
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(content: Text("برجاء تسجيل الدخول مره اخري")),
          );
        } else {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(content: Text("تعذر مسح الحساب: ${e.message}")),
          );
        }
      }
    }
                ...
                _buildSettingsItem(
                  ...
                  onTap: () {
                    Navigator.push(
                      context,
                      MaterialPageRoute(
                        builder: (context) => const Changepasswordarabic(),
                      ),
                    );
                  },
                ),
  ...
  void _showDeleteAccountDialog(BuildContext context) {
    showDialog(
      ...
      builder: (BuildContext context) {
        return AlertDialog(
          title: Center(child: const Text('هل تريد حذف الحساب؟')),
          actions: [
            Row(
              children: [
                Padding(
              ...
              child: Row(
                children: [
                  ...
                  ElevatedButton(
                    ...
                    child: const Text('إلغاء', style: TextStyle(fontSize: 30)),
                  ),
                ],
              ),
            ),
...
void languagedialog(BuildContext context) {
  showDialog(
    ...
    builder: (BuildContext context) {
      return AlertDialog(
        title: Center(
          child: const Text('اللغة', style: TextStyle(fontSize: 30)),
        ),
        content: SizedBox(
          height: 90,
          child: Column(
